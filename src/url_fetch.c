...
void init_url_cache(CAST *vm) {
    if (!vm->compiler.url_cache_dir) {
        ...
        vm->compiler.url_cache_dir = format("%s/cast_cache", temp_dir);
    }
...
static char *get_url_cache_path(CAST *vm, const char *url) {
...
char *fetch_url_to_cache(CAST *vm, const char *url) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "cast-compiler/1.0");
