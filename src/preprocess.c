...
typedef Token *macro_handler_fn(CAST *, Token *);
...
static Token *preprocess2(CAST *vm, Token *tok);
static Macro *find_macro(CAST *vm, Token *tok);
static bool file_exists(char *path);
static char *read_include_filename(CAST *vm, Token **rest, Token *tok, bool *is_dquote, int *out_len);
static long eval_const_expr(CAST *vm, Token **rest, Token *tok);
...
static Token *skip_line(CAST *vm, Token *tok) {
    if (tok->at_bol)
        return tok;
...
static Token *copy_token(CAST *vm, Token *tok) {
...
static Token *new_eof(CAST *vm, Token *tok) {
...
static Hideset *new_hideset(CAST *vm, char *name) {
...
static Hideset *hideset_union(CAST *vm, Hideset *hs1, Hideset *hs2) {
    Hideset head = {};
    Hideset *cur = &head;
...
static Hideset *hideset_intersection(CAST *vm, Hideset *hs1, Hideset *hs2) {
    Hideset head = {};
    Hideset *cur = &head;
...
static Token *add_hideset(CAST *vm, Token *tok, Hideset *hs) {
    Token head = {};
    Token *cur = &head;
...
static Token *append(CAST *vm, Token *tok1, Token *tok2) {
    if (tok1->kind == TK_EOF)
        return tok2;
...
static Token *skip_cond_incl2(CAST *vm, Token *tok, int depth) {
...
static Token *skip_cond_incl(CAST *vm, Token *tok) {
    while (tok->kind != TK_EOF) {
        if (is_hash(tok) &&
            (equal(tok->next, "if") || equal(tok->next, "ifdef") ||
             equal(tok->next, "ifndef"))) {
...
static char *quote_string(CAST *vm, char *str) {
    int bufsize = 3;
    for (int i = 0; str[i]; i++) {
        if (str[i] == '\\' || str[i] == '"')
            bufsize++;
...
static Token *new_str_token(CAST *vm, char *str, Token *tmpl) {
...
static Token *copy_line(CAST *vm, Token **rest, Token *tok) {
    Token head = {};
    Token *cur = &head;
...
static Token *new_num_token(CAST *vm, int val, Token *tmpl) {
    char *buf = format("%d\n", val);
    return tokenize(vm, new_file(vm, tmpl->file->name, tmpl->file->file_no, buf));
}

// Generate comma-separated token sequence from binary data
static Token *generate_embed_tokens(CAST *vm, unsigned char *data, size_t size, Token *tmpl) {
    if (size == 0)
        return NULL;
...
static Token *make_comma_token(CAST *vm, Token *tmpl) {
    Token *comma = copy_token(vm, tmpl);
    comma->kind = TK_PUNCT;
    comma->len = 1;
    comma->loc = ",";
...
static Token *append_tokens(CAST *vm, Token *cur, Token *tokens, Token *tmpl) {
...
static Token *copy_token_list(CAST *vm, Token *tokens, Token *tmpl) {
    if (!tokens)
        return NULL;
...
static Token *read_const_expr(CAST *vm, Token **rest, Token *tok) {
...
static CondIncl *push_cond_incl(CAST *vm, Token *tok, bool included) {
...
static Macro *add_macro(CAST *vm, char *name, int name_len, bool is_objlike, Token *body) {
...
static MacroParam *read_macro_params(CAST *vm, Token **rest, Token *tok, char **va_args_name) {
    MacroParam head = {};
    MacroParam *cur = &head;
...
static void read_macro_definition(CAST *vm, Token **rest, Token *tok) {
    if (tok->kind != TK_IDENT)
...
static MacroArg *read_macro_arg_one(CAST *vm, Token **rest, Token *tok, bool read_rest) {
    Token head = {};
    Token *cur = &head;
    int level = 0;
...
static MacroArg *
read_macro_args(CAST *vm, Token **rest, Token *tok, MacroParam *params, char *va_args_name) {
    Token *start = tok;
    tok = tok->next->next;
...
static char *join_tokens(CAST *vm, Token *tok, Token *end, int *out_len) {
...
static Token *stringize(CAST *vm, Token *hash, Token *arg) {
...
static Token *paste(CAST *vm, Token *lhs, Token *rhs) {
...
static Token *subst(CAST *vm, Token *tok, MacroArg *args) {
    Token head = {};
    Token *cur = &head;
...
static bool expand_macro(CAST *vm, Token **rest, Token *tok) {
...
// Check if a filename is a standard C library header provided by CAST
...
char *search_include_paths(CAST *vm, char *filename, int filename_len, bool is_system) {
    ...
    // Check for embedded standard headers
    if (vm->compiler.use_std_inc) {
    ...
    // For standard library headers, ALWAYS search include_paths (CAST's headers)
    // This prevents accidentally loading system headers which have complex macros
    // that CAST cannot handle
    // NOTE: logic largely superseded by get_std_header check above, but kept for completeness
    // if we add headers not in the embedded set but still "standard" in some way?
    bool force_cast_headers = is_standard_header(filename);

    // For <...> includes:
    //   - Standard headers: search include_paths (CAST's headers)
    //   - Other headers: search system_include_paths (requires -isystem)
    // For "..." includes: search include_paths
    StringArray *paths;
    if (force_cast_headers || !is_system) {
...
static char *search_include_next(CAST *vm, char *filename) {
...
//   ...
//   #endif
static char *detect_include_guard(CAST *vm, Token *tok) {
...
static Token *include_file(CAST *vm, Token *tok, char *path, Token *filename_tok) {
    // Check for "#pragma once"
    if (hashmap_get(&vm->compiler.pragma_once, path))
        return tok;

    // Check if it's an embedded standard header
    char *std_source = vm->compiler.use_std_inc ? get_std_header(path) : NULL;
    if (std_source) {
        Token *tok2 = tokenize_string(vm, path, std_source);
        
        // We don't check include guards for embedded headers as they are typically idempotent or guarded internally
        // But if they use standard guards, we might want to respect them.
        // For now, let's treat them as new token streams.
...
// Read #line arguments
static void read_line_marker(CAST *vm, Token **rest, Token *tok) {
    Token *start = tok;
...
static Token *read_embed_parameter(CAST *vm, Token **rest, Token *tok) {
    Token head = {};
    Token *cur = &head;
    int level = 0;
...
static Token *handle_embed_directive(CAST *vm, Token *tok, Token *directive_start) {
    ...
    while (tok->kind != TK_EOF) {
        ...
        if (equal(tok, "include")) {
            ...
            if (is_url(filename)) {
                ...
                continue;
#else
                error_tok(vm, start->next, "URL includes require CAST to be built with CAST_HAS_CURL=1");
#endif
            }
...
static Macro *add_builtin(CAST *vm, char *name, macro_handler_fn *fn) {
    Macro *m = add_macro(vm, name, strlen(name), true, NULL);
    m->handler = fn;
    return m;
}

static Token *file_macro(CAST *vm, Token *tmpl) {
    while (tmpl->origin)
        tmpl = tmpl->origin;
    return new_str_token(vm, tmpl->file->display_name, tmpl);
}

static Token *line_macro(CAST *vm, Token *tmpl) {
    while (tmpl->origin)
        tmpl = tmpl->origin;
...
static Token *counter_macro(CAST *vm, Token *tmpl) {
...
static Token *timestamp_macro(CAST *vm, Token *tmpl) {
    struct stat st;
...
static char *format_date(CAST *vm, struct tm *tm) {
    static char mon[][4] = {
...
static char *format_time(CAST *vm, struct tm *tm) {
...
// as per the C spec.
static void join_adjacent_string_literals(CAST *vm, Token *tok) {
