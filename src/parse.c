...
static bool is_typename(CAST *vm, Token *tok);
static Type *declspec(CAST *vm, Token **rest, Token *tok, VarAttr *attr);
static Type *typename(CAST *vm, Token **rest, Token *tok);
static Type *enum_specifier(CAST *vm, Token **rest, Token *tok);
static Type *typeof_specifier(CAST *vm, Token **rest, Token *tok);
static Type *typeof_unqual_specifier(CAST *vm, Token **rest, Token *tok);
static Type *type_suffix(CAST *vm, Token **rest, Token *tok, Type *ty);
static Type *declarator(CAST *vm, Token **rest, Token *tok, Type *ty);
static Token *attribute_list(CAST *vm, Token *tok, Type *ty);
static Token *c23_attribute_list(CAST *vm, Token *tok, Type *ty);
static Node *declaration(CAST *vm, Token **rest, Token *tok, Type *basety, VarAttr *attr);
static void array_initializer2(CAST *vm, Token **rest, Token *tok, Initializer *init, int i);
static void struct_initializer2(CAST *vm, Token **rest, Token *tok, Initializer *init, Member *mem);
static void initializer2(CAST *vm, Token **rest, Token *tok, Initializer *init);
static Initializer *initializer(CAST *vm, Token **rest, Token *tok, Type *ty, Type **new_ty);
static Node *lvar_initializer(CAST *vm, Token **rest, Token *tok, Obj *var);
static void gvar_initializer(CAST *vm, Token **rest, Token *tok, Obj *var);
static Node *create_vla_init(CAST *vm, Initializer *init, Type *ty, Obj *var, Token *tok);
static Node *compound_stmt(CAST *vm, Token **rest, Token *tok);
static Node *stmt(CAST *vm, Token **rest, Token *tok);
static Node *expr_stmt(CAST *vm, Token **rest, Token *tok);
static Node *expr(CAST *vm, Token **rest, Token *tok);
static int64_t eval(CAST *vm, Node *node);
static int64_t eval2(CAST *vm, Node *node, char ***label);
static int64_t eval_rval(CAST *vm, Node *node, char ***label);
static bool is_const_expr(CAST *vm, Node *node);
static Node *assign(CAST *vm, Token **rest, Token *tok);
static Node *logor(CAST *vm, Token **rest, Token *tok);
static double eval_double(CAST *vm, Node *node);
static Node *conditional(CAST *vm, Token **rest, Token *tok);
static Node *logand(CAST *vm, Token **rest, Token *tok);
static Node *bitor(CAST *vm, Token **rest, Token *tok);
static Node *bitxor(CAST *vm, Token **rest, Token *tok);
static Node *bitand(CAST *vm, Token **rest, Token *tok);
static Node *equality(CAST *vm, Token **rest, Token *tok);
static Node *relational(CAST *vm, Token **rest, Token *tok);
static Node *shift(CAST *vm, Token **rest, Token *tok);
static Node *add(CAST *vm, Token **rest, Token *tok);
static Node *new_add(CAST *vm, Node *lhs, Node *rhs, Token *tok);
static Node *new_sub(CAST *vm, Node *lhs, Node *rhs, Token *tok);
static Node *mul(CAST *vm, Token **rest, Token *tok);
static Node *cast(CAST *vm, Token **rest, Token *tok);
static Member *get_struct_member(Type *ty, Token *tok);
static Type *struct_decl(CAST *vm, Token **rest, Token *tok);
static Type *union_decl(CAST *vm, Token **rest, Token *tok);
static Node *postfix(CAST *vm, Token **rest, Token *tok);
static Node *funcall(CAST *vm, Token **rest, Token *tok, Node *node);
static Node *unary(CAST *vm, Token **rest, Token *tok);
static Node *primary(CAST *vm, Token **rest, Token *tok);
static Token *parse_typedef(CAST *vm, Token *tok, Type *basety);
static bool is_function(CAST *vm, Token *tok);
static Token *function(CAST *vm, Token *tok, Type *basety, VarAttr *attr);
static Token *global_variable(CAST *vm, Token *tok, Type *basety, VarAttr *attr);
...
static void enter_scope(CAST *vm) {
...
static void leave_scope(CAST *vm) {
    vm->compiler.scope = vm->compiler.scope->next;
}

// Find a variable by name.
static VarScope *find_var(CAST *vm, Token *tok) {
...
static Type *find_tag(CAST *vm, Token *tok) {
    for (Scope *sc = vm->compiler.scope; sc; sc = sc->next) {
        // Linear search through linked list (typically 1-10 entries per scope)
...
static Node *new_node(CAST *vm, NodeKind kind, Token *tok) {
...
static Node *new_binary(CAST *vm, NodeKind kind, Node *lhs, Node *rhs, Token *tok) {
...
static Node *new_unary(CAST *vm, NodeKind kind, Node *expr, Token *tok) {
    Node *node = new_node(vm, kind, tok);
    node->lhs = expr;
    return node;
}

static Node *new_num(CAST *vm, int64_t val, Token *tok) {
...
static Node *new_long(CAST *vm, int64_t val, Token *tok) {
    Node *node = new_node(vm, ND_NUM, tok);
    node->val = val;
    node->ty = ty_long;
    return node;
...
static Node *new_ulong(CAST *vm, long val, Token *tok) {
    Node *node = new_node(vm, ND_NUM, tok);
    node->val = val;
    node->ty = ty_ulong;
    return node;
...
static Node *new_var_node(CAST *vm, Obj *var, Token *tok) {
    Node *node = new_node(vm, ND_VAR, tok);
    node->var = var;
    return node;
}

static Node *new_vla_ptr(CAST *vm, Obj *var, Token *tok) {
...
static VarScope *push_scope(CAST *vm, char *name, int name_len) {
...
static Initializer *new_initializer(CAST *vm, Type *ty, bool is_flexible) {
...
static Obj *new_var(CAST *vm, char *name, int name_len, Type *ty) {
...
static Obj *new_lvar(CAST *vm, char *name, int name_len, Type *ty) {
...
static Obj *new_gvar(CAST *vm, char *name, int name_len, Type *ty) {
    Obj *var = new_var(vm, name, name_len, ty);
...
static Token *skip_to_stmt_end(CAST *vm, Token *tok) {
...
static Token *skip_to_decl_boundary(CAST *vm, Token *tok) {
    int paren_depth = 0;
...
static Type *find_typedef(CAST *vm, Token *tok) {
    if (tok->kind == TK_IDENT) {
...
static void push_tag_scope(CAST *vm, Token *tok, Type *ty) {
...
static Type *func_params(CAST *vm, Token **rest, Token *tok, Type *ty) {
...
static Type *array_dimensions(CAST *vm, Token **rest, Token *tok, Type *ty) {
...
static Type *pointers(CAST *vm, Token **rest, Token *tok, Type *ty) {
...
static Type *abstract_declarator(CAST *vm, Token **rest, Token *tok, Type *ty) {
...
static Node *compute_vla_size(CAST *vm, Type *ty, Token *tok) {
...
static Node *new_alloca(CAST *vm, Node *sz) {
...
static Token *skip_excess_element(CAST *vm, Token *tok) {
    if (equal(tok, "{")) {
...
static void string_initializer(CAST *vm, Token **rest, Token *tok, Initializer *init) {
    if (init->is_flexible)
...
static void array_designator(CAST *vm, Token **rest, Token *tok, Type *ty, int *begin, int *end) {
...
static Member *struct_designator(CAST *vm, Token **rest, Token *tok, Type *ty) {
    Token *start = tok;
    tok = skip(vm, tok, ".");
    if (tok->kind != TK_IDENT)
...
static void designation(CAST *vm, Token **rest, Token *tok, Initializer *init) {
    if (equal(tok, "[")) {
...
static int count_array_init_elements(CAST *vm, Token *tok, Type *ty) {
    bool first = true;
...
static void array_initializer1(CAST *vm, Token **rest, Token *tok, Initializer *init) {
    tok = skip(vm, tok, "{");
...
static void struct_initializer1(CAST *vm, Token **rest, Token *tok, Initializer *init) {
    tok = skip(vm, tok, "{");
...
static void union_initializer(CAST *vm, Token **rest, Token *tok, Initializer *init) {
...
static Type *copy_struct_type(CAST *vm, Type *ty) {
    ty = copy_type(vm, ty);
...
static Node *init_desg_expr(CAST *vm, InitDesg *desg, Token *tok) {
    if (desg->var)
...
static Node *create_lvar_init(CAST *vm, Initializer *init, Type *ty, InitDesg *desg, Token *tok) {
    if (ty->kind == TY_ARRAY) {
        Node *node = new_node(vm, ND_NULL_EXPR, tok);
        for (int i = 0; i < ty->array_len; i++) {
...
static Relocation *
write_gvar_data(CAST *vm, Relocation *cur, Initializer *init, Type *ty, char *buf, int offset) {
    if (ty->kind == TY_ARRAY) {
        int sz = ty->base->size;
        for (int i = 0; i < ty->array_len; i++)
...
static Node *asm_stmt(CAST *vm, Token **rest, Token *tok) {
...
// to bitfields.
static Node *to_assign(CAST *vm, Node *binary) {
...
static void collect_captures_in_node(CAST *vm, Node *node, Obj *outer_locals, 
...
static Node *block_literal(CAST *vm, Token **rest, Token *tok) {
    Token *start = tok;
    tok = tok->next;  // Skip ^
...
static void struct_members(CAST *vm, Token **rest, Token *tok, Type *ty) {
    Member head = {};
    Member *cur = &head;
    int idx = 0;
...
static Type *struct_union_decl(CAST *vm, Token **rest, Token *tok) {
...
static Node *struct_ref(CAST *vm, Node *node, Token *tok) {
    add_type(vm, node);
...
static Node *new_inc_dec(CAST *vm, Node *node, Token *tok, int addend) {
    add_type(vm, node);
...
static Node *funcall(CAST *vm, Token **rest, Token *tok, Node *fn) {
    add_type(vm, fn);
...
//               | "default" ":" assign
static Node *generic_selection(CAST *vm, Token **rest, Token *tok) {
    Token *start = tok;
    tok = skip(vm, tok, "(");
...
//         | "__cast_types_compatible_p" "(" type-name, type-name, ")"
//         | "__cast_reg_class" "(" type-name ")"
//         | ident
//         | str
//         | num
static Node *primary(CAST *vm, Token **rest, Token *tok) {
    ...
    if (equal(tok, "__cast_types_compatible_p")) {
        tok = skip(vm, tok->next, "(");
    ...
    if (equal(tok, "__cast_compare_and_swap")) {
    ...
    if (equal(tok, "__cast_atomic_exchange")) {
    ...
    // In CAST's simplified model, blocks are already heap-allocated, so this just returns the block
    ...
    // In CAST's simplified model, this is a no-op (VM teardown handles cleanup)
...
static void create_param_lvars(CAST *vm, Type *param) {
    if (param) {
...
static void resolve_goto_labels(CAST *vm) {
...
static Obj *find_func(CAST *vm, char *name, int name_len) {
...
static void mark_live(CAST *vm, Obj *var) {
...
static void scan_globals(CAST *vm) {
    Obj head;
    Obj *cur = &head;
...
static void declare_builtin_functions(CAST *vm) {
    // alloca(size) -> void*
